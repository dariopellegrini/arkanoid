...
/* Funzione di connessione al DB. */
	...
	function dbConnect(){
		...
		mysql_select_db("") or die("Errore nell'accesso al DB: " . mysql_error());
		return $connection;
		}
...
//--------------------------------------------------------------------------------------------------------------------------
	...
	/* Funzione che ritorna vero o falso a seconda che le credenziali siano presenti nel DB. */
		...
		$sql = "SELECT username,password FROM utenti WHERE username=\"".$username."\" AND password=MD5(\"".$password."\")";
		...
		while($riga = mysql_fetch_row($risposta)){
			if($riga[0]!=null && $riga[1]!=null) $b=true;
...
/* Inserisce un nuovo utente nel DB. */
		...
		function insertUser($username, $password, $name, $surname){
			...
			$sql = "INSERT INTO utenti( username, password, nome, cognome, immagine ) VALUES(\"".$username."\",MD5(\"".$password."\"),\"".					$name."\",\"".$surname."\",\"immagini/default.png\")";
			mysql_query($sql) or die("Errore durante la query SQL: " . mysql_error());
			mysql_close($connection);
